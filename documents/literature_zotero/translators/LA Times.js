{
	...
	"target": "^https?://(?:www\\.|travel\\.|articles\\.)?latimes\\.com",
	"minVersion": "2.1.9",
	"maxVersion": null,
	"priority": 100,
	"inRepository": true,
...
attachments      : [{ url: FW.Url(),
  title:  "LA Times Snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//div[contains(@id, "byline")]/span[3]').text().replace(/By|,.+/g, "").split(/\s+and\s+/).cleanAuthor("author"),
date             : FW.Xpath('//span[@class="pubdate"]/a').text(),
...
choices          : {
  titles :  FW.Xpath('//ul[contains(@class, "results-list")]//h3/a').text().trim(),
  urls    :  FW.Xpath('//ul[contains(@class, "results-list")]//h3/a').key("href")
}
});
/** BEGIN TEST CASES **/
var testCases = [
	...
	{
		"type": "web",
		"url": "http://www.latimes.com/search/dispatcher.front?Query=argentina&target=adv_article",
		"items": "multiple"
	}
]
/** END TEST CASES **/