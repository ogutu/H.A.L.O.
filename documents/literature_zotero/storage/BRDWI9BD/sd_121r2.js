...
var InfoBubble = {
  ...
  typeVals:["Art", "Ref", "Cite"],
  bubblesArtFilled:0,
  bubblesRefFilled:0,
  bubblesCiteFilled:0,
  bubbleTimeout: 1000,
  bubbleShowDelay: 1000,
  currentBubble: null,
  currentLink: null,
  showTimerID: null,
  hideTimerID: null,

  init: function(typeVal) {
      var tdIndex = 1;
      var td = jQuery('#mlkt'+typeVal+'TD_' + tdIndex);
      jQuery('div[id^=mlkt'+typeVal+'TD_]').each(function(i,e) {
        $(e).bind('mouseover', {tv:typeVal, ti:i+1}, function(event) {
          InfoBubble.get(SDM.pii, event.data.tv, event.data.ti);
        });
        $(e).mouseout(function() {
          InfoBubble.timeout();
        });
  ...
  get: function(pii, typeVal, bubbleNum) {
    ...
    var bubbleUrl="";
    if(typeVal == "Cite") {
        var bubbleUrl = SDM.urlPrefix +'/citedby/bubble/'+SDM.pii;
    }
    else {
        bubbleUrl = SDM.urlPrefix +'/mlkt/bubble/'+SDM.pii+'/'+typeVal;
    }
    bubbleList.load(
      bubbleUrl,
      function() {
...
//      bLink.toggleClass("mlktLinkOn mlktLinkOff");
      bLink.addClass("mlktLinkOn");
      bLink.removeClass("mlktLinkOff");
//      aBubble.toggleClass("infobubble infobubbleOn");
      aBubble.addClass("infobubbleOn");
      aBubble.removeClass("infobubble");
...
//      this.currentLink.toggleClass("mlktLinkOn mlktLinkOff");
      this.currentLink.addClass("mlktLinkOff");
      this.currentLink.removeClass("mlktLinkOn");
//      this.currentBubble.toggleClass("infobubbleOn infobubble");
      this.currentBubble.addClass("infobubble");
      this.currentBubble.removeClass("infobubbleOn");
