...
itime <- 1
if (nt > 1) {
# determine the index of the selected time step
  timeout <- as.Date(paste(year,"-",month,"-1",sep=""))
  itime <- which(date.time==timeout)

# check if the time is included in the file
  if (length(itime) == 0) stop("Selected time not included in the file!")
  }
